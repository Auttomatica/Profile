

const misProyectos = document.querySelector('.mis-proyectos');
const menu = document.getElementById('menuPrincipal');
const focoEnMisProyectos = document.getElementById('proyectos');

eventListener();


function eventListener(){

misProyectos.addEventListener('click', verMisProyectos);
//menu.addEventListener('click', verOpcionesMenu);

}


// DELEGATION PARA MIS PROYECTOS
function verMisProyectos(e){

    e.preventDefault();

   if(e.target.classList.contains('verProyecto')){
       
   }

    ;





}


 //function verOpcionesMenu(e){

   // e.preventDefault();

    
   // if(e.target.classList.contains('btnMisProyectos')){

   //     document.getElementById('proyectos').focus();

  //  }



//}

function getFocus(){

    document.getElementById('proyectos').focus();

}